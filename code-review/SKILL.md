...
`main`, `HEAD~5`, etc. If they did not specify one, infer the repository's
primary upstream branch when unambiguous; otherwise ask.

Capture the review scope once:

- committed branch changes: `git diff <fixed-point>...HEAD`
- staged changes: `git diff --cached`
- unstaged changes: `git diff`
- untracked paths: `git status --short` (read only relevant files)
...
- All scope commands and the commit list.
